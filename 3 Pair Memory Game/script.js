$(document).ready(function() {
    ...
    for (i = 0; i < images.length; i++) {
        ...
        three = numbersArray[Math.floor(Math.random() * numbersArray.length)];
        qTwo = Math.floor(three / 6);
        rTwo = Math.floor(three % 6);

        var removeItem = three;
        ...
        $('#game_table tr:eq(' + qTwo + ') td:eq(' + rTwo + ')').append($(images[i]).clone());
    }
    ...
    $('#game_table tr td').on('click', function(event) {
        if (x == true) {
            ...
            if (clickCounter == 3) {
                $(this).find('img').show();
                thirdclicked = $(this).find('img');
                x = false;
                compare();

            }

            if ($('td').find('img:visible').length == 54) {
    ...
    function compare() {
        ...
        if ((lastclicked.attr("src") != secondclicked.attr("src")) ||
            (secondclicked.attr("src") != thirdclicked.attr("src")) ||
            (lastclicked.parent()[0] === secondclicked.parent()[0]) ||
            (secondclicked.parent()[0] === thirdclicked.parent()[0])) {
